...
from f_mnist_gan_model import Generator, Discriminator
...
# Initialize generator and discriminator (true 1-channel FMNIST)
generator = Generator(latent_dim, num_classes, img_channels=1).to(device)
...
# Keep 32x32 spatial size for current GAN architecture, but stay true 1-channel.
...
subset_indices = indices[:num_samples // 10]
...
for epoch in range(epochs):
    ...
    if fake_images is not None:
        os.makedirs("gen_ims", exist_ok=True)
        save_image(fake_images[:25], f'gen_ims/generated_images_epoch_{epoch+1}.png', nrow=5, normalize=True)
    torch.save(generator.state_dict(), 'fmnist_generator.pth')  # Save the generator's state dictionary
